...
public:
	...
	void update(float secFromLastUpdate, float gx, float gy, float gz, float ax,
			float ay, float az) {
		float recipNorm;
		float s0, s1, s2, s3;
		...
		q0 += qDot1 * secFromLastUpdate;
		q1 += qDot2 * secFromLastUpdate;
		q2 += qDot3 * secFromLastUpdate;
		q3 += qDot4 * secFromLastUpdate;
